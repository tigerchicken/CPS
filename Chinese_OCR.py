...
tf.app.flags.DEFINE_string('train_data_dir', './dataset/train_new/', 'the train dataset dir')
tf.app.flags.DEFINE_string('test_data_dir', './dataset/test_new/', 'the test dataset dir')
tf.app.flags.DEFINE_string('log_dir', './log', 'the logging dir')
#tf.app.flags.DEFINE_boolean('restore', False, 'whether to restore from checkpoint')
tf.app.flags.DEFINE_boolean('restore', False, 'whether to restore from checkpoint')
tf.app.flags.DEFINE_boolean('epoch', 1, 'Number of epoches')
tf.app.flags.DEFINE_boolean('batch_size', 40, 'Validation batch size')
#tf.app.flags.DEFINE_boolean('batch_size', 12, 'Validation batch size')
...
def build_graph(top_k):
    ...
    with tf.device('/gpu:5'):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            conv1_1 = slim.conv2d(images, 64, [3, 3], 1, padding='SAME', scope='conv1_1')
            conv1_2 = slim.conv2d(conv1_1, 64, [3, 3], 1, padding='SAME', scope='conv1_2')
            max_pool_1 = slim.max_pool2d(conv1_2, [2, 2], [2, 2], padding='SAME', scope='pool1')

            conv2_1 = slim.conv2d(max_pool_1, 128, [3, 3], padding='SAME', scope='conv2_1')
            conv2_2 = slim.conv2d(conv2_1, 128, [3, 3], padding='SAME', scope='conv2_2')
            max_pool_2 = slim.max_pool2d(conv2_2, [2, 2], [2, 2], padding='SAME', scope='pool2')

            conv3_1 = slim.conv2d(max_pool_2, 256, [3, 3], padding='SAME', scope='conv3_1')
            conv3_2 = slim.conv2d(conv3_1, 256, [3, 3], padding='SAME', scope='conv3_2')
            conv3_3 = slim.conv2d(conv3_2, 256, [3, 3], padding='SAME', scope='conv3_3')
            max_pool_3 = slim.max_pool2d(conv3_3, [2, 2], [2, 2], padding='SAME', scope='pool3')

            conv4_1 = slim.conv2d(max_pool_3, 512, [3, 3], padding='SAME', scope='conv4_1')
            conv4_2 = slim.conv2d(conv4_1, 512, [3, 3], padding='SAME', scope='conv4_2')
            conv4_3 = slim.conv2d(conv4_2, 512, [3, 3], padding='SAME', scope='conv4_3')
            max_pool_4 = slim.max_pool2d(conv4_3, [2, 2], [2, 2], padding='SAME', scope='pool4')

            conv5_1 = slim.conv2d(max_pool_4, 512, [3, 3], padding='SAME', scope='conv5_1')
            conv5_2 = slim.conv2d(conv5_1, 512, [3, 3], padding='SAME', scope='conv5_2')
            conv5_3 = slim.conv2d(conv5_2, 512, [3, 3], padding='SAME', scope='conv5_3')
            max_pool_5 = slim.max_pool2d(conv5_3, [2, 2], [2, 2], padding='SAME', scope='pool5')

            flatten = slim.flatten(max_pool_5)
            fc6 = slim.fully_connected(slim.dropout(flatten, keep_prob), 4096,
                                       activation_fn=tf.nn.relu, scope='fc6')
            fc7 = slim.fully_connected(slim.dropout(fc6, keep_prob), 4096,
                                       activation_fn=tf.nn.relu, scope='fc7')
            logits = slim.fully_connected(slim.dropout(fc7, keep_prob), FLAGS.charset_size, activation_fn=None,
                                          scope='fc8')
...
def train():
    ...
    # 填好数据读取的路径
    train_feeder = DataIterator(data_dir='./dataset/train_new/')
    test_feeder = DataIterator(data_dir='./dataset/test_new/')
